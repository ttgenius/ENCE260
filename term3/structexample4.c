 ...
 */
Student* readOneStudent(FILE* fp)
{
    char buffer[MAX_LINE_LENGTH];  // Buffer into which we read a line from stdin
    Student* sp = NULL;           // Pointer to a dynamically allocated student record

    // Read a line, extract name and age

    char* cp = fgets(buffer, MAX_LINE_LENGTH, fp);
